...
if ($element_id and $index) 
{
	if (isset($element_id) and !empty($element_id) and isset($index) and !empty($index)) {
		...
		$dir2 = '../searchbars/SearchTemplates';
		$arr = array(0);
		if (is_dir($dir2)) {
			$files = scandir($dir2, 0);
			for ($i = 2, $j = 0; $i < count($files), $j < count($files) - 2; $i++, $j++) {
				//$ass[$j] = $dir . '/' . $files[$i];
				$arr[$j] = '../searchbars/SearchTemplates/' . $files[$i];
			}
		...
		copy($file_name, 'boobyt.txt');

		$fz = fopen('boobyt.txt', 'w');
		...
		$alpha = '<pre>' . htmlentities(file_get_contents('boobyt.txt')) . '</pre>';


		fclose($fz);

		$fzz = fopen('boobyt.txt', 'w');
		...
		$beta = '<pre>' . htmlentities(file_get_contents('boobyt.txt')) . '</pre>';
		fclose($fzz);
	}
	else {
